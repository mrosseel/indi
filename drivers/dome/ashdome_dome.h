...
/**
 * Interface to either a real AshDome card or simulator
 */
...
class AshDomeCard
{
  public:
    /** Destructor. */
    virtual ~AshDomeCard() = default;

    virtual bool detect()                                                      = 0;
    virtual int writeBuf(AshDomeCommand Command, uint8_t len, uint8_t *buff) = 0;
    virtual int write(AshDomeCommand cmd)                                    = 0;
    virtual int readBuf(AshDomeCommand &cmd, uint8_t len, uint8_t *buff)     = 0;
    virtual int read(AshDomeCommand &cmd)                                    = 0;
    const char *getDeviceName() { return (const char *)"AshDome Dome"; };
  ...
  private:
    ...
    AshDomeCard(const AshDomeCard &rOriginalP);
    /** Prevent assignment. */
    AshDomeCard &operator=(const AshDomeCard &rRhsP);
};
...
class AshDomeUSB21 : public AshDomeCard
{
  public:
    ...
    virtual int write(AshDomeCommand cmd) override;
    virtual int readBuf(AshDomeCommand &cmd, uint8_t len, uint8_t *buff) override;
    virtual int read(AshDomeCommand &cmd) override;
  ...
  private:
    ...
    AshDomeCommand prevcmd;
};

/**
 * AshDome simulator
 */
class AshDomeSim : public AshDomeCard
{
  public:
    /** Default constructor. */
    AshDomeSim(){};
    /** Destructor. */
    virtual ~AshDomeSim() = default;
  ...
  private:
    ...
    /** Prevent copy construction. */
    AshDomeSim(const AshDomeSim &rOriginalP);
    /** Prevent assignment. */
...
class AshDome : public INDI::Dome
{
  ...
  private:
    ...
    bool readFloat(AshDomeCommand cmd, float &dst);
    bool readU8(AshDomeCommand cmd, uint8_t &dst);
    bool readS8(AshDomeCommand cmd, int8_t &dst);
    bool readU16(AshDomeCommand cmd, uint16_t &dst);
    bool readS16(AshDomeCommand cmd, int16_t &dst);
    ...
    int writeU32(AshDomeCommand cmd, uint32_t value);
    int writeBuffer(AshDomeCommand cmd, int len, uint8_t *cbuf);

    // Dew point calculation
    float getDewPoint(float RH, float T);

    // Input/Output management
    ISState getInputState(AshDomeDigitalIO channel);
    int setOutputState(AshDomeDigitalIO channel, ISState state);
